...
while True:
	...
	entrar = True if (minutos >= 0.03 and minutos <= 0.06) or (minutos >= 5.03 and minutos <= 5.06) else False
		...
		if dir:
			...
			while i < martingale:
				...
				if i > 3:	
					...
					while True:
						try:
							minutos2 = float(((datetime.now()).strftime('%M.%S'))[1:])
							entrar2 = True if (minutos2 >= 0.03 and minutos2 <= 0.06) or (minutos2 >= 5.03 and minutos2 <= 5.06) else False
							if entrar2:
					...
					while True:
						...
						if status:
							...
							if valor > 0:
								...
								f = open("bot_mhi_sem_gale_losses.txt", "a")
								f.write(dados)
								f.write('\n')
								f.close()
