...
def VerificaOperacao(penta, total, velas, qtd_velas, paridade, maxima, minima):
	...
	if paridade[7:10] == 'OTC':
		if vela_atual[0]['max'] >= maxima and vela_atual[0]['open'] < vela_atual[0]['close'] and velas[qtd_velas-2] == 'g':
			estrategia = 'Vela Maxima / Vela atual eh a maxima entre 120 velas = put'
			result = [paridade, 'put', penta, estrategia]

		elif vela_atual[0]['min'] <= minima and vela_atual[0]['open'] > vela_atual[0]['close'] and velas[qtd_velas-2] == 'r':
		...
		if vela_atual[0]['max'] >= maxima and velas[qtd_velas-2] == 'g':
			estrategia = 'Vela Maxima / Vela atual eh a maxima entre 60 velas = put'
			result = [paridade, 'put', penta, estrategia]

		elif vela_atual[0]['min'] <= minima and velas[qtd_velas-2] == 'r':
			estrategia = 'Vela Minima / Vela atual eh minima entre 60 velas = call'
			result = [paridade, 'call', penta, estrategia]
...
while True:
	minutos = float(((datetime.now()).strftime('%M.%S'))[2:])
	entrar = True if (minutos >= 0.03 and minutos <= 0.1) else False
		...
		if dir:
			...
			while i < martingale:
				...
				if i >= 3:
					...
					f = open("horarios_gale_bot_test.txt", "a")
					f.write(dados)
					f.write('\n')
					f.close()
				...
				if i > 3:
					...
					while True:
						try:
							...
							entrar2 = True if (minutos2 >= 0.03 and minutos2 <= 0.1) else False
							if entrar2:
